				...
				success: function(response) {
					if (response) {
						for (var i = 0; i < response.length; i++) {
							...
							item.find('td.schedules p:eq(2) span').html(response[i][7]);
							item.find('td.schedules p:eq(3) span').html(response[i][5]);
							item.find('td.schedules p:eq(4) span').html(response[i][6]);
							item.find('td.schedules p:eq(5) span').html(response[i][8]);
