...
class Tocbot extends \WP_Widget{

    //小工具slug
	protected $widget_slug = 'b2-widget-toc';

    //短代码名
	protected static $shortcode = 'b2_widget_toc';
	...
	public function __construct() {
		parent::__construct(
			...
			array(
				...
				'description' => __( '“文章内页自动导航”小工具', 'b2' ),
			)
		);
	...
	public static function get_widget( $atts ) {
		...
		if ( empty( $widget )) {
			...
			if($instance['toc_title']){
				$widget .= '<div class="b2-widget-title">';
				$widget .= $atts['before_title']. esc_attr( $instance['toc_title'] ) .$atts['after_title'];
				$widget .= '</div>';
			}
