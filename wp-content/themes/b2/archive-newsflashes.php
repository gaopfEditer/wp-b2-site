...
<div class="b2-single-content wrapper">
    <div id="primary-home" class="wrapper content-area">
        <main class="site-main">
            <div class="news-content" ref="paged" data-paged="<?php echo $paged; ?>">
                <div id="post-news" :class="['modal',showPostFrom ? 'show-modal' : '']" v-cloak>
                    <div class="modal-content">
                        <span class="close-button" @click="showPostFrom = !showPostFrom">×</span>
                        ...
                        <div class="post-news b2-radius mg-b b2-pd">
                            ...
                            <div class="news-footer">
                                <button @click="submitNewsflashes" :disabled="locked" :class="locked ? 'b2-loading' : ''"><?php echo __('发布','b2').$name; ?></button>
                                <?php if(!empty($tags)){ ?>
                                    <label class="news-tags" data-tag="<?php echo $tags[0]; ?>" ref="tag">
                ...
                <div class="box b2-radius newsflashes-list">
                    <div class="newsflahses-cover" style="background-image:url(<?php echo b2_get_thumb(array('thumb'=>b2_get_option('newsflashes_main','newsflashes_cover'),'width'=>800,'height'=>200)); ?>)">
                        <div class="n-desc">
                            <div class="n-desc-name">
                                <h1><?php echo $name; ?></h1>
                                <p><?php echo b2_get_option('newsflashes_main','newsflashes_desc'); ?></p>
                            </div>
                            <div class="po-n">
                    ...
                    <div class="news-list-box">
                        <div>
                            <?php 
                                $data = Newsflashes::get_newsflashes_data($paged);
                                            ...
                                            $html .= '<li id="news-item-'.$_v['id'].'">
                                                ...
                                                <div class="news-item-content">
                                                    <div class="news-item-h">
                                                        ...
                                                        '.($_v['img'] ? b2_get_img(array('src'=>B2_DEFAULT_IMG,'alt'=>$_v['title'],'class'=>array('news-item-img','b2-radius'))) : '').'
                                                    </div>
                                                    <div class="new-meta">
                    ...
                    <div class="b2-pagenav <?php echo $data['pages'] <= 1 ? 'b2-hidden-always' : ''; ?> " data-max="<?php echo $data['pages']; ?>">
                        <page-nav ref="goldNav" paged="<?php echo $paged; ?>" navtype="json" pages="<?php echo $data['pages']; ?>" type="p" :box="selecter" :opt="opt" :api="api" url="<?php echo get_post_type_archive_link('newsflashes').(isset($_GET['action']) ? '?action=showbox' : ''); ?>" title="<?php echo b2_get_option('newsflashes_main','newsflashes_tdk_title'); ?>" @return="get"></page-nav>
                    </div>
                </div>
            </div>
        </main>
